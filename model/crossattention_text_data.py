...
class TextDataCrossAttention(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super(TextDataCrossAttention, self).__init__()
    ...
    def forward(self, text_features, time_features):
        #计算注意力分数
        query = self.query_proj(text_features).unsqueeze(1) #.unsqueeze(dim) 会在指定的维度 dim 上插入一个大小为 1 的维度
        ...
        fused_features = attended_features.squeeze(1) + text_features
